 ...
 */
package main.java.com.bradrydzewski.gwtgantt;
...
public class DateUtil {
	...
	public static int differenceInMinutes(Date endDate, Date startDate) {
		int difference = 0;
		if(!areOnTheSameMinute(endDate, startDate)) {
			int endDateOffset = -(endDate.getTimezoneOffset() * 60 * 1000);
			long endDateInstant = endDate.getTime() + endDateOffset;
			int startDateOffset = -(startDate.getTimezoneOffset() * 60 * 1000);
			long startDateInstant = startDate.getTime() + startDateOffset;
			double differenceDouble = (double) Math.abs(endDateInstant - startDateInstant) / (double) MILLIS_IN_A_MINUTE;
			differenceDouble = Math.max(1.0D, differenceDouble);
			difference = (int) Math.round(differenceDouble);//added math.ceil .. then changed to floor... shit, which one???
		}
		return difference;
	...
	public static boolean areOnTheSameMinute(Date dateOne, Date dateTwo) {
		return areOnTheSameDay(dateOne, dateTwo)
				&& dateOne.getHours() == dateTwo.getHours()
				&& dateOne.getMinutes() == dateTwo.getMinutes();
	}
