...
import main.java.com.bradrydzewski.gwtgantt.connector.CalculatorFactory;
import main.java.com.bradrydzewski.gwtgantt.geometry.Point;
...
import de.bachelor.smartSchedules.client.presenter.GanttManager;
...
public class GanttChartPresenter<T> extends TaskDisplayPresenter<T> {
    ...
    protected static final int TASK_ROW_HEIGHT = 22;
    ...
    protected void renderBackground() {
        ...
        for (int i = 0; i < weeks; i++) {
            ...
            for (int d = 0; d < DateUtil.DAYS_PER_WEEK; d++) {
                ...
                /*
                 * Hintergrund bei Sonntag und Samtag
                 * 
...
//        int collapseLevel = -1;
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            */
             ...
             * NEU
             */
            renderTask(task, provider.getLevel(task));
        }
    }
    ...
    protected void renderTask(T task, int level) {
        ...
        int left = (int)((double)(DateUtil.differenceInMinutes(start, provider.getStart(task))/(60.0*24.0)) * ROW2_WIDTH_OFFSET);
        int width = (int)((double)(DateUtil.differenceInMinutes(provider.getStart(task), provider.getFinish(task))/(60.0*24.0)) * ROW2_WIDTH_OFFSET - 2);
    ...
    protected void renderConnectors(List<? extends T> values) {
    	if(!GanttManager.getShowConnectors())
    		return;
