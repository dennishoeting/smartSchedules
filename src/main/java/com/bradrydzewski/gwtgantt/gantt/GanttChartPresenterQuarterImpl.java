...
public class GanttChartPresenterQuarterImpl<T> extends GanttChartPresenter<T> {
    ...
    protected void renderTask(T task, int level) {
        ...
        int left = (int)((double)(DateUtil.differenceInMinutes(start, provider.getStart(task))/(60.0*24.0)) * UNIT_WIDTH);
        int width = (int)((double)(DateUtil.differenceInMinutes(provider.getStart(task), provider.getFinish(task))/(60.0*24.0)) * UNIT_WIDTH-2);
